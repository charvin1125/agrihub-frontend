...
//   try {
//     const { id } = req.params; // Get the billId from URL

//     console.log("Fetching bill with ID:", id); // 🔍 Debugging

//     const bill = await Bill.findOne({ billId: id }); // 🔍 Check if Bill exists
...
//     if (!order) {
//       console.log("Order not found for bill:", bill.orderId);
//     }

//     res.json({
//       ...bill.toObject(),
//       products: order ? order.products : [], // ✅ Ensure products are included
//     });

//   } catch (error) {
//     console.error("Error fetching bill details:", error); // 🔥 Log the full error
//     res.status(500).json({ error: "Internal Server Error" });
//   }
// };
exports.getBillById = async (req, res) => {
