...
//           withCredentials: true,
//         });
//         console.log("Navbar user fetch response:", response.data); // Debug log
//         setUser(response.data);
//       } catch (error) {
//         console.error("Fetch user error:", error.response?.data || error.message);
//         setUser(null); // Reset user if fetch fails (e.g., not logged in)
//       }
//     };
//     fetchUser();
...
// Ensure axios sends credentials with all requests
...
const NavigationBar = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        const response = await axios.get("https://agrihub-backend-pvfl.onrender.com/api/users/profile");
        console.log("Navbar user fetch response:", response.data);
        setUser(response.data);
      } catch (error) {
  ...
  const handleLogout = async () => {
    try {
      await axios.post("https://agrihub-backend-pvfl.onrender.com/api/users/logout");
      setUser(null);
      localStorage.removeItem("cart");
      setCartCount(0);
      handleMenuClose();
