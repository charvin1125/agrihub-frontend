...
//   //   doc.addImage(Logo, "PNG", 20, 30, 50, 50);
//   //   doc.setFontSize(12);
//   //   doc.text(`Order ID: ${order._id}`, 80, 40);
//   //   doc.text(`Customer Name: ${order.name}`, 80, 50);
//   //   doc.text(`Phone: ${order.phone}`, 80, 60);
...
//   //   doc.text(`Status: ${order.status}`, 80, 120);
//   //   doc.text(`Payment Method: ${order.paymentMethod}`, 80, 130);
//   //   doc.text(`Total Amount (with GST): ₹${calculateGrandTotalWithGST(order.cart)}`, 80, 140);
//   //   doc.text(`Is Due: ${order.isDue ? "Yes" : "No"}`, 80, 150);
...
//   //     doc.text(`Price: ₹${item.price.toFixed(2)}`, 130, yPos);
//   //     doc.text(`GST: ${item.gst || 0}%`, 160, yPos);
//   //     doc.text(`Total (with GST): ₹${calculateTotalWithGST(item)}`, 180, yPos);
//   //     yPos += 10;
//   //   });
//   //   doc.text(`Grand Total (with GST): ₹${calculateGrandTotalWithGST(order.cart)}`, 180, yPos + 10);

//   //   doc.addImage(Seal, "PNG", 160, yPos + 20, 50, 50);
//   //   doc.save(`invoice_${order._id}.pdf`);
//   //   console.log("PDF downloaded successfully:", `invoice_${order._id}.pdf`);
//   // };
// // Replace the existing handleDownloadPDF function with this improved version
// const handleDownloadPDF = () => {
//   if (!order) return;
...
//   doc.text('AgriHub Service Invoice', 60, 20);
...
//   doc.text(`Invoice #: ${order._id}`, column1X, detailsStartY);
//   doc.text(`Date: ${formatDate(order.createdAt)}`, column2X, detailsStartY);
//   doc.text(`Customer: ${order.name}`, column1X, detailsStartY + 6);
//   doc.text(`Phone: ${order.phone}`, column2X, detailsStartY + 6);
//   doc.text(`Address: ${order.address || 'N/A'}`, column1X, detailsStartY + 12);
//   doc.text(`Pincode: ${order.pincode || 'N/A'}`, column2X, detailsStartY + 12);
//   doc.text(`Payment: ${order.paymentMethod}`, column1X, detailsStartY + 18);
//   doc.text(`Status: ${order.status}`, column2X, detailsStartY + 18);

//   // Table Header
//   const tableStartY = 65;
//   doc.setFillColor(primaryColor);
//   doc.rect(10, tableStartY, 190, 8, 'F');
//   doc.setTextColor(255, 255, 255);
//   doc.setFontSize(11);
//   doc.setFont('helvetica', 'bold');
  
//   const headers = ['Item', 'Size', 'Qty', 'Price', 'GST%', 'Total'];
//   const columnWidths = [70, 30, 20, 25, 20, 25];
//   let currentX = 10;
...
//           if (i === 0) {
//               const splitText = doc.splitTextToSize(data, columnWidths[i] - 4);
//               splitText.forEach((line, lineIndex) => {
//                   doc.text(line, currentX + 2, yPos + (lineIndex * 5));
//               });
//           } else {
...
//   doc.line(10, totalsY - 5, 200, totalsY - 5);
  
//   doc.setFont('helvetica', 'bold');
//   doc.text('Grand Total (with GST):', 150, totalsY);
//   doc.text(`₹${calculateGrandTotalWithGST(order.cart)}`, 180, totalsY, { align: 'right' });
  
//   // Footer with Seal
//   doc.addImage(Seal, 'PNG', 160, totalsY + 10, 40, 40);
//   doc.setFontSize(8);
//   doc.setTextColor(secondaryColor);
//   doc.text(`Generated on: ${formatDate(new Date())}`, 10, 285);
//   doc.text('© AgriHub - All rights reserved', 90, 285, { align: 'center' });
