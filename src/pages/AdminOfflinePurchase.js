...
//         "https://agrihub-backend-xu19.onrender.com/api/orders/offline-purchase",
//         orderData,
...
//   // Handle product selection
//   const handleProductSelect = (product, variant, quantity) => {
//     if (quantity <= 0 || isNaN(quantity) || quantity > (variant.stock || 0)) return;

//     const gstRate = variant.gst ? variant.gst / 100 : 0;
//     const priceWithGST = (variant.sellingPrice || 0) * (1 + gstRate) * quantity;
...
//   // Handle offline purchase
//   const handlePurchase = async () => {
//     if (!customerName || !phone || !crop || selectedProducts.length === 0) {
...
//                               <TextField
//                                 type="number"
//                                 inputProps={{ min: 1, max: variant.stock || 0 }}
//                                 value={existingItem ? existingItem.quantity : ""}
//                                 onChange={(e) => handleProductSelect(product, variant, parseInt(e.target.value, 10))}
...
//               {selectedProducts.map((item) => (
//                 <Typography key={item.variantId} variant="body2" sx={{ color: "text.primary" }}>
//                   {item.name || "N/A"} ({item.size || "N/A"}) - {item.quantity || 0} pcs: ₹{(item.totalWithGST || 0).toFixed(2)}
//                 </Typography>
//               ))}
//             </DialogContent>
//             <DialogActions>
...
//   // Get the oldest available batch for a variant
//   const getOldestBatch = (variant) => {
//     const availableBatches = (variant.batches || []).filter((batch) => batch.stock > 0);
//     return availableBatches.sort((a, b) => new Date(a.addedDate) - new Date(b.addedDate))[0];
//   };

//   // Handle product selection with batch
//   const handleProductSelect = (product, variant, quantity) => {
//     const oldestBatch = getOldestBatch(variant);
//     if (!oldestBatch || quantity <= 0 || isNaN(quantity) || quantity > oldestBatch.stock) return;

//     const gstRate = oldestBatch.gst / 100;
//     const priceWithGST = oldestBatch.sellingPrice * (1 + gstRate) * quantity;

//     setSelectedProducts((prevProducts) => {
//       const existingIndex = prevProducts.findIndex((p) => p.batchId === oldestBatch._id);
//       if (existingIndex !== -1) {
//         const updatedProducts = [...prevProducts];
//         updatedProducts[existingIndex].quantity = quantity;
//         updatedProducts[existingIndex].totalWithGST = priceWithGST;
...
//             name: product.name || "Unknown Product",
//             size: variant.size || "N/A",
//             batchNumber: oldestBatch.batchNumber || "N/A",
//             price: oldestBatch.sellingPrice || 0,
//             gst: oldestBatch.gst || 0,
//             stock: oldestBatch.stock || 0,
//             quantity,
//             totalWithGST: priceWithGST,
//           },
//         ];
...
//         setCrop("");
//         setPaymentMethod("Cash");
//         fetchProducts(); // Refresh products to reflect updated stock
//       } else {
//         alert(response.data.message || "Purchase failed. Try again.");
//       }
//     } catch (error) {
...
//                       (product.variants || []).map((variant) => {
//                         const oldestBatch = getOldestBatch(variant);
//                         if (!oldestBatch) return null; // Skip if no available batches

//                         const existingItem = selectedProducts.find((p) => p.batchId === oldestBatch._id);
//                         return (
//                           <TableRow key={`${variant._id}-${oldestBatch._id}`} sx={{ "&:hover": { bgcolor: darkMode ? "#2E7D32" : "#F1F8E9" } }}>
//                             <TableCell sx={{ color: "text.primary" }}>{product.name || "N/A"}</TableCell>
//                             <TableCell sx={{ color: "text.primary" }}>{variant.size || "N/A"}</TableCell>
//                             <TableCell align="right" sx={{ color: "text.primary", display: { xs: "none", sm: "table-cell" } }}>
//                               {oldestBatch.batchNumber || "N/A"}
//                             </TableCell>
//                             <TableCell align="right" sx={{ color: "text.primary", display: { xs: "none", sm: "table-cell" } }}>
//                               ₹{(oldestBatch.sellingPrice || 0).toFixed(2)}
//                             </TableCell>
//                             <TableCell align="right" sx={{ color: "text.primary", display: { xs: "none", md: "table-cell" } }}>
//                               {oldestBatch.gst || 0}
//                             </TableCell>
//                             <TableCell align="right">
//                               <Chip
//                                 label={`${oldestBatch.stock || 0}`}
//                                 color={oldestBatch.stock <= 5 ? "error" : "success"}
//                                 size={isMobile ? "small" : "medium"}
//                               />
//                             </TableCell>
//                             <TableCell align="right">
//                               <TextField
//                                 type="number"
//                                 inputProps={{ min: 1, max: oldestBatch.stock || 0 }}
//                                 value={existingItem ? existingItem.quantity : ""}
//                                 onChange={(e) => handleProductSelect(product, variant, parseInt(e.target.value, 10))}
//                                 disabled={oldestBatch.stock === 0}
//                                 variant="outlined"
//                                 size={isMobile ? "small" : "medium"}
//                                 sx={{ width: isMobile ? "60px" : "80px" }}
//                               />
...
//   const [sidebarOpen, setSidebarOpen] = useState(false);
//   const [isMobile, setIsMobile] = useState(window.innerWidth < 600);
//   const [selectedBatchIds, setSelectedBatchIds] = useState({}); // New state for tracking batch selections

//   const filteredProducts = products.filter((p) =>
//     p.name?.toLowerCase().includes(searchQuery.toLowerCase())
//   );

//   // Fetch products and initialize selectedBatchIds
//   useEffect(() => {
//     fetchProducts();
...
//       const res = await axios.get("https://agrihub-backend-xu19.onrender.com/api/product/list", { withCredentials: true });
//       const fetchedProducts = res.data || [];
//       console.log("Fetched Products:", JSON.stringify(fetchedProducts, null, 2)); // Debug log
//       setProducts(fetchedProducts);

//       // Initialize selectedBatchIds with the oldest batch for each variant
//       const initialBatchIds = {};
//       fetchedProducts.forEach((product) => {
//         (product.variants || []).forEach((variant) => {
//           const sortedBatches = getSortedBatches(variant);
//           if (sortedBatches.length > 0) {
//             initialBatchIds[variant._id] = sortedBatches[0]._id; // Default to oldest batch
//           }
//         });
//       });
...
//   // Get batches sorted by batch number
//   const getSortedBatches = (variant) => {
//     return (variant.batches || [])
//       .filter((batch) => batch.stock > 0)
//       .sort((a, b) => a.batchNumber.localeCompare(b.batchNumber)); // Sort by batchNumber
//   };
...
//     try {
//       const response = await axios.post(
//         "https://agrihub-backend-xu19.onrender.com/api/orders/offline",
//         orderData,
//         { withCredentials: true }
//       );

//       console.log("Purchase Response:", JSON.stringify(response.data, null, 2)); // Debug log
...
//                       (product.variants || []).map((variant) => {
//                         const sortedBatches = getSortedBatches(variant);
//                         if (sortedBatches.length === 0) return null; // Skip if no available batches
...
//   try {
//     const res = await axios.get(`https://agrihub-backend-xu19.onrender.com/api/orders/customer/${phoneNumber}`, { withCredentials: true });
//     const previousOrder = res.data;
//     if (previousOrder) {
//       setCustomerName(previousOrder.name || "");
//       setAddress(previousOrder.address || "");
//       setPincode(previousOrder.pincode || "");
//       setCrop(previousOrder.crop || "");
//     }
//   } catch (error) {
//     console.log("No previous order found for this phone number:", error.response?.data || error.message);
...
//   const [isMobile, setIsMobile] = useState(window.innerWidth < 600);
//   const [selectedBatchIds, setSelectedBatchIds] = useState({});
//   const [isFetchingCustomer, setIsFetchingCustomer] = useState(false); // New: Loading state for customer fetch
...
//   const fetchCustomerDetails = async (phoneNumber) => {
//     if (!/^\d{10}$/.test(phoneNumber)) return; // Validate 10 digits
//     setIsFetchingCustomer(true);
//     try {
//       const res = await axios.get(`https://agrihub-backend-xu19.onrender.com/api/orders/customer/${phoneNumber}`, { withCredentials: true });
//       const { data } = res.data; // Adjusted to match backend response structure
//       if (data) {
...
//       }
//     } catch (error) {
//       console.log("No previous order found:", error.response?.data?.message || error.message);
//       if (error.response?.status === 404) {
//         setCustomerName("");
//         setAddress("");
//         setPincode("");
...
//       fetchCustomerDetails(phone);
//     } else {
//       // Clear fields if phone number is incomplete
//       setCustomerName("");
//       setAddress("");
//       setPincode("");
//       setReferenceName("");
