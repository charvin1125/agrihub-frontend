...
const AdminServices = () => {
  ...
  const fetchServices = async () => {
    ...
    try {
      const [servicesRes, userRes] = await Promise.all([
        axios.get("https://agrihub-backend-xu19.onrender.com/api/services/list", { withCredentials: true }),
        axios.get("https://agrihub-backend-xu19.onrender.com/api/users/me", { withCredentials: true }),
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`https://agrihub-backend-xu19.onrender.com/api/services/update/${editId}`, formData, {
          withCredentials: true,
        });
        alert("Service updated successfully!");
      } else {
        await axios.post("https://agrihub-backend-xu19.onrender.com/api/services/add", formData, {
          withCredentials: true,
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this service?")) {
      try {
        await axios.delete(`https://agrihub-backend-xu19.onrender.com/api/services/delete/${id}`, { withCredentials: true });
                      ...
                      services.map((serviceItem) => (
                        <TableRow key={serviceItem._id} sx={{ "&:hover": { bgcolor: darkMode ? "#2e2e2e" : "#f5f5f5" } }}>
                          ...
                          <TableCell align="right">
                            {serviceItem.image ? (
                              <img
                                src={`https://agrihub-backend-xu19.onrender.com/${serviceItem.image}`}
