...
import { Table, Button, Container, Alert } from "react-bootstrap"; // Using Bootstrap for styling
...
const ManageCustomers = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await axios.get("https://agrihub-backend-xu19.onrender.com/api/users/customers", {
  ...
  const handleDelete = async (customerId) => {
    if (window.confirm("Are you sure you want to delete this customer?")) {
      try {
        ...
        await axios.delete(`https://agrihub-backend-xu19.onrender.com/api/users/${customerId}`, {
