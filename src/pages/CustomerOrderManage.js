...
//     setLoading(true);
//     try {
//       const response = await axios.get(`https://agrihub-backend-xu19.onrender.com/api/orders/customer-orders/${mobile}`);
...
//     setLoading(true);
//     try {
//       const response = await axios.get(`https://agrihub-backend-xu19.onrender.com/api/orders/customer-orders/${mobile}`, { withCredentials: true });
//       const sortedOrders = response.data.orders.sort(
//         (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
//       );
//       setOrders(sortedOrders);
...
//     setLoading(true);
//     try {
//       const response = await axios.get(`https://agrihub-backend-xu19.onrender.com/api/orders/customer/${mobile}`, { withCredentials: true });
//       const sortedOrders = response.data.orders.sort(
//         (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
//       );
//       setOrders(sortedOrders);
...
//     try {
//       const newStatus = currentDueStatus ? "Paid" : "Pending"; // Toggle between Paid (clear due) and Pending (set due)
//       const response = await axios.put(
//         `https://agrihub-backend-xu19.onrender.com/api/orders/update-status/${orderId}`,
//         { status: newStatus },
...
//   // Fetch admin details by userId (only for offline orders)
//   // const fetchAdminDetails = async (userId) => {
//   //   try {
//   //     const response = await axios.get(`https://agrihub-backend-xu19.onrender.com/api/users/${userId}`, { withCredentials: true });
//   //     setAdminDetails(response.data); // Assuming response.data has { firstName, lastName, email }
//   //   } catch (error) {
...
//     setAdminDetails(response.data); // Adjust to response.data.data if needed
//   } catch (error) {
...
//   setSelectedOrder(order);
//   if (order.purchaseType === "Offline" && order.userId) {
//     console.log("Using populated userId for offline order:", order.userId);
//     setAdminDetails(order.userId); // Already populated
//   } else {
//     console.log("Opening bill for online order:", order._id);
//     setAdminDetails(null);
//   }
//   setOpenBill(true);
// };

// // In  ... rest of the code remains the same, but remove fetchAdminDetails if populated
...
//       if (selectedOrder.purchaseType === "Offline" && adminDetails) {
//         doc.text(`Ordered By: ${adminDetails.firstName} ${adminDetails.lastName || ""}`, 20, 160);
//       }

//       doc.text("Products:", 20, selectedOrder.purchaseType === "Offline" && adminDetails ? 170 : 160);
//       selectedOrder.cart.forEach((item, index) => {
//         doc.text(
//           `- ${item.productId?.name || "Unknown"}: ${item.quantity} pcs, ₹${item.totalWithGST.toLocaleString()}`,
//           30,
//           (selectedOrder.purchaseType === "Offline" && adminDetails ? 180 : 170) + index * 10
//         );
//       });
...
//                     <strong>Purchase Type:</strong> {selectedOrder.purchaseType}
//                   </Typography>
//                   {selectedOrder.purchaseType === "Offline" && adminDetails ? (
//                     <>
//                       <Divider sx={{ my: 2, bgcolor: darkMode ? "#b0b0b0" : "#757575" }} />
//                       <Typography variant={isMobile ? "subtitle2" : "h6"} sx={{ mb: 1, color: "text.primary" }}>
//                         Ordered By:
...
//                 variant="contained"
//                 startIcon={<DownloadIcon />}
//                 disabled={!selectedOrder || (selectedOrder.purchaseType === "Offline" && !adminDetails)}
//               >
//                 Download PDF
//               </Button>
