...
//     setLoading(true);
//     try {
//       const response = await axios.get(`https://agrihub-backend-tz4v.onrender.com/api/orders/customer-orders/${mobile}`);
...
const CustomerOrderManage = () => {
  ...
  const fetchOrders = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`https://agrihub-backend-tz4v.onrender.com/api/orders/customer-orders/${mobile}`, { withCredentials: true });
      const sortedOrders = response.data.orders.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
      );
      setOrders(sortedOrders);
