...
//         const response = await axios.post("https://agrihub-backend-xu19.onrender.com/api/orders/place", orderData, {
//           withCredentials: true,
//         });
...
//           "https://agrihub-backend-xu19.onrender.com/api/orders/create-razorpay-order",
...
//                 "https://agrihub-backend-xu19.onrender.com/api/orders/verify-razorpay-payment",
//                 {
...
//   const totalAmount = cart.reduce((sum, item) => {
//     const price = parseFloat(item.price) || 0; // `price` is `sellingPrice` from ProductDetails.js
//     const quantity = parseInt(item.quantity, 10) || 1;
//     const gstRate = parseFloat(item.gst) ? parseFloat(item.gst) / 100 : 0;
//     return sum + price * quantity * (1 + gstRate);
//   }, 0).toFixed(2);
...
//       totalAmount: parseFloat(totalAmount),
//       cart: cart.map((item) => ({
//         productId: item._id, // Matches ProductDetails.js
//         variantId: item.variantId,
//         name: item.name,
//         size: item.variantSize || "N/A",
//         price: parseFloat(item.price), // `sellingPrice`
//         quantity: parseInt(item.quantity, 10),
//         gst: parseFloat(item.gst) || 0,
//       })),
//     };
...
//   // Theme configuration aligned with previous pages
//   const theme = createTheme({
//     palette: {
...
//     const storedCart = JSON.parse(localStorage.getItem("cart")) || [];
//     console.log("Checkout cart loaded:", storedCart); // Debug log
//     setCart(storedCart);
...
//   const totalAmount = cart.reduce((sum, item) => {
//     const price = parseFloat(item.price) || 0; // `sellingPrice` from batch
//     const quantity = parseInt(item.quantity, 10) || 1;
//     const gstRate = parseFloat(item.gst) ? parseFloat(item.gst) / 100 : 0;
//     return sum + price * quantity * (1 + gstRate);
//   }, 0).toFixed(2);
...
//         name: item.name,
//         size: item.size || "N/A",
//         price: parseFloat(item.price), // `sellingPrice` from batch
...
//     console.log("Order data being sent to backend:", orderData); // Debug log

//     try {
//       if (paymentMethod === "Pay Later" || paymentMethod === "Cash") {
//         const response = await axios.post("https://agrihub-backend-xu19.onrender.com/api/orders/place", orderData, { withCredentials: true });
//         if (response.data.success) {
//           alert("Order placed successfully!");
//           const savedDataKey = `savedCustomerData_${user.username}`;
//           localStorage.setItem(savedDataKey, JSON.stringify({ customerName, phone, pincode, address }));
