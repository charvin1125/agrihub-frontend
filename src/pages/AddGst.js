...
const AddGST = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await axios.get(`https://your-backend.onrender.com/api/product/${id}`);
  ...
  const handleUpdateGST = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`https://your-backend.onrender.com/api/product/update-gst/${id}`, { gst }, { withCredentials: true });
