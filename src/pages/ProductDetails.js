...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await axios.get(`https://agrihub-backend-pvfl.onrender.com/api/product/${id}`, {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `https://agrihub-backend-pvfl.onrender.com/api/review/products/${id}/product-review`,
        reviewData,
        { withCredentials: true }
      );
          ...
          <Grid container spacing={{ xs: 2, sm: 3, md: 4 }}>
            <Grid item xs={12} md={6}>
              <Card>
                <CardMedia
                  component="img"
                  image={selectedImage ? `https://agrihub-backend-pvfl.onrender.com/${selectedImage}` : "https://via.placeholder.com/400x400?text=No+Image"}
                  alt={product.name}
                  sx={{
                  ...
                  <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap", justifyContent: "center" }}>
                    {product.images.map((img, index) => (
                      <Box
                        key={index}
                        component="img"
                        src={`https://agrihub-backend-pvfl.onrender.com/${img.url}`}
                        alt={`Thumbnail ${index}`}
                        sx={{
                          width: { xs: 50, md: 70 },
                          height: { xs: 50, md: 70 },
