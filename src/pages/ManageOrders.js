...
const ManageOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get("https://agrihub-backend-pvfl.onrender.com/api/orders/list", {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      const response = await axios.put(
        `https://agrihub-backend-pvfl.onrender.com/api/orders/update-status/${orderId}`,
        { status: newStatus },
        { withCredentials: true }
      );
  ...
  const handlePayDues = async (orderId) => {
    try {
      const response = await axios.put(
        `https://agrihub-backend-pvfl.onrender.com/api/orders/pay-dues/${orderId}`,
