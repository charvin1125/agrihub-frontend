...
//       try {
//         const response = await axios.get("https://agrihub-backend-xu19.onrender.com/api/orders/list", {
...
//           order._id === orderId ? { ...order, status: newStatus, statusHistory: response.data.order.statusHistory } : order
//         ));
//       }
//     } catch (error) {
...
//     try {
//       const response = await axios.put(
//         `https://agrihub-backend-xu19.onrender.com/api/orders/pay-dues/${orderId}`,
//         { status: "Paid", duesPaidDate: new Date() }, // Send current date as dues payment date
//         { withCredentials: true }
//       );
//       if (response.data.success) {
//         setOrders(orders.map((order) =>
//           order._id === orderId ? { ...order, status: "Paid", isDue: false, duesPaidDate: response.data.order.duesPaidDate, statusHistory: response.data.order.statusHistory } : order
//         ));
...
//                           />
//                         </TableCell>
//                         <TableCell>{formatDate(order.duesPaidDate)}</TableCell>
//                         <TableCell>
//                           {order.statusHistory?.length > 0 ? (
//                             order.statusHistory.map((history, index) => (
//                               <Typography key={index} variant="body2">
