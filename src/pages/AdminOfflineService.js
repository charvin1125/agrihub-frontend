...
// import Sidebar from "../components/Sidebar"; // Assuming this is the same Sidebar component
...
//     setLoading(true);
//     try {
//       const res = await axios.get("https://agrihub-backend-xu19.onrender.com/api/services", { withCredentials: true });
...
//     try {
//       const res = await axios.get(`https://agrihub-backend-xu19.onrender.com/api/bookings/customer/${phoneNumber}`, { withCredentials: true });
//       const bookings = res.data.bookings; // Assuming similar structure to orders
//       if (bookings && bookings.length > 0) {
//         const latestBooking = bookings[0];
//         setCustomerName(latestBooking.customerName || "");
//         setAddress(latestBooking.farmAddress || "");
...
//     const totalPrice = service.price * areaInSqFt;
...
//     if (!customerName || !phone || !address || !pincode || !crop || selectedServices.length === 0) {
...
//         "https://agrihub-backend-xu19.onrender.com/api/bookings/offline",
//         bookingData,
...
//       } else {
//         alert(response.data.message || "Booking failed. Try again.");
//       }
//     } catch (error) {
//       console.error("Error booking offline service:", error.response?.data || error.message);
//       alert("Failed to book service. Check console for details.");
//     }
//   };
...
//                       <TableCell align="right">Price (per Sq Ft)</TableCell>
...
//               <Typography variant="body1" sx={{ color: "text.primary" }}><strong>Farm Address:</strong> {address}</Typography>
...
//     // Calculate total price based on pricePer100SqFt
//     const pricePerSqFt = service.pricePer100SqFt / 100;
//     const totalPrice = pricePerSqFt * areaInSqFt;
...
//     // Since your bookService endpoint handles one service per booking,
//     // we'll create separate bookings for each selected service
//     try {
//       const bookingPromises = selectedServices.map(async (item) => {
//         const bookingData = {
//           userId: "admin-offline", // Placeholder; adjust based on your auth logic
//           serviceId: item.serviceId,
...
//         return axios.post("https://agrihub-backend-xu19.onrender.com/api/bookings", bookingData, { withCredentials: true });
//       });
  ...
  const handleBookService = async () => {
    ...
    try {
      const bookingPromises = selectedServices.map(async (item) => {
        ...
        return axios.post("https://agrihub-backend-xu19.onrender.com/api/book-service", bookingData, { withCredentials: true });
      });
