...
//                   desc: "Manage your crops effectively with our tools.", 
//                   icon: <AgricultureIcon />, 
//                   link: "/crop-management" // Add this
//                 },
//                 { title: "Weather Updates", desc: "Real-time weather updates for planning.", icon: <CloudIcon /> ,},
//                 { title: "Market Insights", desc: "Stay informed about market trends.", icon: <TrendingUpIcon /> },
//               ].map((feature, index) => (
...
//     { title: "Welcome to AgriHub", description: "Your one-stop solution for agriculture management.", image: SlideImg1 },
//     { title: "Grow Smarter", description: "Leverage technology for better yields.", image: SlideImg2 },
//     { title: "Sustainable Farming", description: "Building a greener future.", image: SlideImg3 }, // Updated empty slide
//   ];
...
//             <Grid container spacing={4}>
//               {[
//                 { title: "Crop Management", desc: "Manage your crops effectively with our tools.", icon: <AgricultureIcon />, link: "/crop-management" },
//                 { title: "Weather Updates", desc: "Real-time weather updates for planning.", icon: <CloudIcon />, link: "/weather-updates" },
//                 { title: "Market Insights", desc: "Stay informed about market trends.", icon: <TrendingUpIcon />, link: "/market-insights" },
//               ].map((feature, index) => (
//                 <Grid item xs={12} sm={6} md={4} key={index}>
//                   <Card
//                     sx={{ bgcolor: "#E8F5E9", textAlign: "center", cursor: "pointer" }}
...
//   // State for planned crops
//   const [plannedCrops, setPlannedCrops] = useState([]);

//   // Load crops and check for new product notifications on mount
//   useEffect(() => {
//     const savedCrops = localStorage.getItem("plannedCrops");
//     if (savedCrops) {
//       const crops = JSON.parse(savedCrops);
//       setPlannedCrops(crops);
//       checkNewProductNotifications(crops);
//     }
//   }, []);

//   // Check for new crops added within the last 2 days
//   const checkNewProductNotifications = (crops) => {
//     const now = new Date();
//     const twoDaysInMs = 2 * 24 * 60 * 60 * 1000; // 2 days in milliseconds

//     crops.forEach((crop) => {
//       const addedDate = new Date(crop.id); // Assuming id is a timestamp from Date.now() in CropPlanning.js
...
//         toast.success(
//           `New Crop Added: ${crop.name}! Added on ${addedDate.toLocaleDateString()}. Notification active for ${daysLeft} more day(s).`,
//           {
//             toastId: `new-crop-${crop.id}`, // Unique ID to prevent duplicates
//             autoClose: 7000, // Longer duration for visibility
//           }
//         );
//       }
//     });
...
//   // Load new products and check for notifications on mount
//   useEffect(() => {
...
//   // Check for new products added within the last 2 days
//   const checkNewProductNotifications = (products) => {
//     const now = new Date();
//     const twoDaysInMs = 2 * 24 * 60 * 60 * 1000; // 2 days in milliseconds

//     products.forEach((product) => {
//       const addedDate = new Date(product.id); // id is a timestamp from Date.now()
//       const timeSinceAdded = now - addedDate;

//       if (timeSinceAdded <= twoDaysInMs) {
//         const daysLeft = Math.ceil((twoDaysInMs - timeSinceAdded) / (24 * 60 * 60 * 1000));
//         toast.success(
//           `New Product Added: ${product.name}! Added on ${addedDate.toLocaleDateString()}. Available for ${daysLeft} more day(s).`,
//           {
//             toastId: `new-product-${product.id}`, // Unique ID to prevent duplicates
//             autoClose: 7000, // Longer duration for visibility
//           }
//         );
//       }
